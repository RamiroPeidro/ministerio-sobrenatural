// Script en formato ESM para permitir importaciones de fetch
import fs from 'fs';
...
// Configuración - Valores ya configurados con tus datos
...
async function createClerkUser(firstName, lastName, email, password) {
  try {
    ...
    const securePassword = "MinisterioSN_2025!"; // Contraseña segura para todos
    
    // Generar un nombre de usuario basado en nombre y apellido
    const normalizedFirstName = firstName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // Eliminar acentos
    const normalizedLastName = lastName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // Eliminar acentos
    // Usar guión bajo en lugar de punto, y eliminar cualquier caracter que no sea letra, número, guión o guión bajo
    ...
    // Usar el formato exacto que espera Clerk según los errores anteriores
    const userData = {
      username,
      email_address: email,  // Cambiado de email_addresses a email_address directo
