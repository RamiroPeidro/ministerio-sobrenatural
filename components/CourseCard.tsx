...
interface CourseCardProps {
  ...
  buttonText?: string;
  buttonHref?: string; // Nueva propiedad para la URL del botón
}
...
export function CourseCard({ 
  ...
  // Si no se proporciona buttonHref, usamos href como fallback
  const buttonLink = buttonHref || href;
  ...
  return (
    <div className="bg-card rounded-xl overflow-hidden shadow-lg transition-all duration-300 ease-in-out hover:shadow-xl hover:translate-y-[-4px] border border-border flex flex-col flex-1">
      ...
      <Link
        href={href}
        prefetch={false}
        className="group hover:no-underline relative h-52 w-full overflow-hidden"
      >
        {course.image ? (
          <Image
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="space-y-4 mt-auto">
          ...
          {/* Botón "Ver más información" con URL personalizable */}
          {showViewButton && (
